...
/*   Updated: 2024/07/10 17:22:09 by rdi-marz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	this->_isDuplicate["errors"] = false;
	this->_isDuplicate["listen"] = false;
	this->_isDuplicate["maxbdy"] = false;
	this->_isDuplicate["snames"] = false;
}
...
ServerConfiguration::addDirective(Server &server, const int lineNumber, const string &directive)
{
	...
	if (key == "errors_page") {
		_handleErrorsPage(server, lineNumber, value);
	} else if (key == "listen") {
		_handleListen(server, lineNumber, value);
	} else if (key == "max_body_size") {
		_handleMaxBodySize(server, lineNumber, value);
	} else if (key == "server_name") {
		_handleServerName(server, lineNumber, value);
	} else {
...
void
ServerConfiguration::_handleErrorsPage(Server &server, const int lineNumber, const string &value)
{
...
void
ServerConfiguration::_handleListen(Server &server, const int lineNumber, const string &value)
{
...
void
ServerConfiguration::_handleMaxBodySize(Server &server, const int lineNumber, const string &value)
{
...
bool
ServerConfiguration::_contains(const vector<string>& vec, const string& value) {
...
void
ServerConfiguration::_handleServerName(Server &server, const int lineNumber, const string &value)
{
	...
	while (ss >> name)
	{
		...
		else if (!_contains(server.snames, name)) {
			server.snames.push_back(name);
		}
		else {
