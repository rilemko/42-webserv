...
/*   Updated: 2024/07/26 19:25:37 by mconreau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Server::~Server()
{
	Logger::info("Closing server: " + this->listen.first + ":" + this->listen.second);
...
Server::run()
{
	...
	for (size_t i = 0, j = this->routes.size() - 1; this->routes.size() && i < j; i++)
	{
...
Server::match(Request &request) const
{
	...
	if ((this->target.first == 0 || this->target.first == addr.sin_addr.s_addr) && this->target.second == addr.sin_port)
	{
		...
		const string	name(host.substr(0, host.find(':')));

		for (int j = 0; j < i; j++)
		{
			if (String::match(this->snames[j], name))
				return (true);
		}
	}
	return (false);
